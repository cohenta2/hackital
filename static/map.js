...
var graph_dict = new Map();

function grab_subreddits(list) {
  subreddit_list = list;
}

function add_data_to_map(x, y, j) {
  data = subreddit_list[j];
  graph_dict.set(x.toString() + "," + y.toString(), {'name': data['name'], 'subs': data['subs']})
}

// function build_map_dictionary(list, x, y) {
...
function mover(d) {
  var el = d3.select(this)
    ...
    coordinates = x + ',' + y;
    console.log(graph_dict[coordinates]);
...
function build_hexagons() {
  ...
  for (var i = 0; i < MapRows; i++) {
      for (var j = 0; j < MapColumns; j++) {
          ...
          add_data_to_map(hexRadius * j * 1.75, hexRadius * i * 1.5, j)
      }//for j
  }//for i
